...
lrnng_rate_plhdr = tf.placeholder(tf.float32, (), name='learning_rate')
...
d_a_opt = tf.train.AdamOptimizer(learning_rate=lrnng_rate_plhdr, beta1=beta1).minimize(sum_d_loss_a, var_list=disc_a_vars)
d_b_opt = tf.train.AdamOptimizer(learning_rate=lrnng_rate_plhdr, beta1=beta1).minimize(sum_d_loss_b, var_list=disc_b_vars)
g_opt   = tf.train.AdamOptimizer(learning_rate=lrnng_rate_plhdr, beta1=beta1).minimize(sum_g_loss  , var_list=gnrt_vars  )
...
for e in range(epoch):
	for itr in range(iter_num):
		learning_rate = learning_rate * 1 / np.power(2, e//4) # Half learning rate in every 4 epochs
		# Get batch
		...
		# Optimize
		_, batch_g_loss   = sess.run([g_opt, sum_g_loss], feed_dict={a_real_img_plhdr:batch_a, b_real_img_plhdr:batch_b, lrnng_rate_plhdr:learning_rate})
		_, batch_d_loss_b = sess.run([d_b_opt, sum_d_loss_b], feed_dict={b_real_img_plhdr:batch_b, a_b_histry_plhdr:batch_a_b_hist, lrnng_rate_plhdr:learning_rate})
		_, batch_d_loss_a = sess.run([d_a_opt, sum_d_loss_a], feed_dict={a_real_img_plhdr:batch_a, b_a_histry_plhdr:batch_b_a_hist, lrnng_rate_plhdr:learning_rate})
...
saver.save(sess, '../../models/v6_CycleGAN/' + dataset + '_gen.ckpt')
sess.close()
