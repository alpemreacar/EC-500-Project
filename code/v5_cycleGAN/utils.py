...
# Read image and resize it and transform it to (-1., 1.)

def img_read_resize(path, height, width):
	# Read a [0, 255] image
	img = scipy.misc.imread(path).astype(np.uint8)
	img_resized = scipy.misc.imresize(img, [height, width], 'bilinear').astype(np.float32)
	return (img_resized * 2.) / 255. - 1.
# Merge images
